...
class FlaskTestcase(unittest.TestCase):
    ...
    def test_availability_create_user(self):
        ...
        response = tester.post('/user', data=dict(rollno='rollno'), content_type='application/json')
        self.assertTrue('Roll no already exist!!!', response.data)

    def test_create_user(self):
        tester = app.test_client(self)
        response = tester.post('/user',data=json.dumps({'name':'name','rollno':'rollno','address':'address','branch':'branch'}),
                                   content_type='application/json')
        self.assertTrue('Data store successfully!!!',response.data)
    ...
    def test_get_user(self):
        ...
        rv = tester.get('/user', content_type='application/xml')
    ...
    def test_modify_user(self):
        tester = app.test_client(self)
        response = tester.put ( '/user', data=json.dumps (
            {'name': 'name', 'rollno': 'rollno', 'address': 'address', 'branch': 'branch'} ),
                                 content_type='application/json' )
        self.assertTrue ( 'Data store successfully!!!', response.data )
